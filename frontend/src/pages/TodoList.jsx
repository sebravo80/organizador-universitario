...
import { getPendings, createPending, updatePending, deletePending } from '../services/pendingService';
...
const TodoList = () => {
  ...
  const loadItems = async () => {
    try {
      ...
      const errorMsg = err.msg || err.message || 'Error al cargar los pendientes';
      setError(errorMsg);
      setItems([]);
    } finally {
      setLoading(false);
  ...
  const handleAddItem = async (e) => {
    ...
    try {
      ...
      await createPending(pendingData);
      ...
      const errorMsg = err.msg || err.message || 'Error al añadir el pendiente';
      setError(errorMsg);
    }
  };
  ...
  const handleDeleteItem = async (id) => {
    try {
      ...
      const errorMsg = err.msg || err.message || 'Error al eliminar el pendiente';
      setError(errorMsg);
    }
  };
  ...
  const handleToggleComplete = async (item) => {
    try {
      ...
      const response = await updatePending(item._id, updatedItem);
      
      // Actualizar el item en la lista
      setItems(items.map(i => i._id === item._id ? response : i));
      setSuccess(`Pendiente marcado como ${response.completed ? 'completado' : 'pendiente'}`);
    } catch (err) {
      console.error('Error al actualizar pendiente:', err);
      const errorMsg = err.msg || err.message || 'Error al actualizar el pendiente';
      setError(errorMsg);
    }
  };
